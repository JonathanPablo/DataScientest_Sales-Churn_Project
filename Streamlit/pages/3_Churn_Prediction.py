...
if add_radio == "Introduction":
    ...
    st.write('Therefore the following columns where extracted from the company\'s ERP System and got partly calculated in SQL:')

    if st.button('Show Contract Column Descriptions') or st.session_state.CD:
        st.session_state.CD = True # set to keep showing
        # Open image of ColumnDescriptions
        image_path = st.session_state.image_folder + 'ColumnDescriptions.png'
        try:
            image = Image.open(image_path)
            st.image(image, caption='Descriptions of Contracts df')
    ...
    st.write('1. What are main global impacts on contract terminations?')
    st.write('2. Which contrcats have the highest probability to get terminated by the customer?')
...
if add_radio == "Preprocessing":
   ...
   if "products_loaded" not in st.session_state:
       st.session_state.products_loaded = False
   if "ppO" not in st.session_state:
       st.session_state.ppO = False
   ...
   if st.session_state.contracts_loaded:
       ...
       if not st.session_state.contracts_preprocessed or (st.button("Show Preprocessing Options") or st.session_state.ppO):
             # Open image of prepro options
            image_path = st.session_state.image_folder + 'DataPreparation.png'
            try:
                image = Image.open(image_path)
                st.image(image, caption='Example of Preprocession options')
            except FileNotFoundError:
                st.error(f"Image file '{image_path}' not found. Please check the file path.")
