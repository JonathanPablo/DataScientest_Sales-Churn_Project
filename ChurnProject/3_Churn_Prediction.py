...
#set title & configs
st.set_page_config(page_title="03_Churn Prediction",layout="wide")
...
if add_radio == "Modelling":
    ...
    if train_test_button or st.session_state.model_sel:
        ...
        with st.sidebar:         
            ...
            selected_model = st.selectbox("Select Classifier:", ["XGBoost", "RandomForest"])
...
if add_radio == "Probabilities":
    ...
    if proba_button or st.session_state.proba:
        ...
        shap_values_top_n = explainer(st.session_state.X_test_top_n)
        
        st.subheader(f'SHAP Waterfall plot for top {n} contracts')
        
        col1, col2 = st.columns((2,1)) #reduce plot size
        # show interactive waterfall plot
        x = col1.slider('Waterfall example row:', 0, n-1, step=1, value = 0)
        update_waterfall_plot(x, shap_values = shap_values_top_n)
        col1.pyplot(bbox_inches='tight', use_container_width = True)
